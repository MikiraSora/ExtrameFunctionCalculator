...
public class BooleanCaculator {
    ...
    String boolOperatorSymbol="< > == <= >= || && !=";
    private boolean isBooleanOperator(Calculator.Symbol op){
        return boolOperatorSymbol.contains(op.Solve());
    }

    private boolean isSameType(Calculator.Expression a, Calculator.Expression b){
        if(a.GetType()!=b.GetType())
            return false;
        return true;
    }

    private boolean isCalculatable(Calculator.Expression expression){
